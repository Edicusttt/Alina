...
module.exports = {
  ...
  run: (client, message, [channel='']) => list.findById(message.guild.id, (err, doc) => {
    ...
    if (!channel || !['text', 'news'].includes(channel.type)){
      return message.channel.send(`${em.error} | **${message.member.displayName}**, please provide a valid channel ID or channel mention.`);
    } else if (!channel.permissionsFor(message.guild.me).has('SEND_MESSAGES')){
      return message.channel.send(`${em.error} | **${message.member.displayName}**, I need you to give me permission to **Send Messages** on ${channel} and try again.`);
    } else if (!channel.permissionsFor(message.guild.me).has('EMBED_LINKS')){
      return message.channel.send(`${em.error} | **${message.member.displayName}**, I need you to give me permission to **Embed Links** on ${channel} and try again.`);
    };
    ...
    .then(() => {
      ...
      return message.channel.send(`${em.success} | Successfully set the anime airing notification channel to ${channel}!`)
