  ...
  run: (client, message, [ role ]) => guilds.findById(message.guild.id, (err, doc) => {
    ...
    if (!role){
      if (!guildprofile.roles.muted){
        return message.channel.send(`${em.error} | **${message.author.tag}**, Muterole not set!`);
      } else {
        const role = message.guild.roles.cache.get(guildprofile.roles.muted);
        if (!role){
          return message.channel.send(`${em.error} | **${message.author.tag}**, Could not find this server's designated muterole!`);
        } else {
      ...
      if (!role){
        return message.channel.send(`${em.error} | **${message.author}**, Invalid Role - Please supply the mention of the role, the ID of the role, or its Role Name.`)
      } else {
        ...
        .then(() => {
          ...
          return message.channel.send(`${em.success} | **${message.author.tag}**, Successfully set the muterole to ${role}!`);
