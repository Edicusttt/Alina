...
module.exports = {
  ...
  adminOnly: true,
  group: 'Setup',
  description: 'Adds a new anime to watch for new episodes of. You may provide an AniList entry link or a MyAnimeList link Supports multiple ids/links',
  requiresDatabase: true,
  parameters: [ 'Anilist/alina link' ],
  examples: [
  ...
  if (err){
    ...
    for (const [key, ids] of Object.entries({ id, idMal })){
      ...
      if (query.errors){
        return message.channel.send(`${em.error} | Unable to connect to AniList. Please try again later`);
      };
    ...
    if (!res.length){
      return message.channel.send(`${em.error} | None of the provided links/ids matches anime from AniList!`);
    };
    ...
    .then(() => message.channel.send(
      ...
      .setFooter(`Watch | \©️${new Date().getFullYear()} Alina`)
      .addFields(res.splice(0,25).sort((A,B) => B.id - A.id).map(entry => {
        ...
        return {
          name: '\u200b', value: [
            ['\\⚠️ Failed to add', ` ${em.acm} Successfully added`][Number(filter)],
