...
module.exports = {
  ...
  run: (client, message) => guilds.findById(message.guild.id, (err, doc) => {
    ...
    if (!channels.length){
      return message.channel.send(`${em.error} | **${message.member.displayName}**, Please mention the channel(s) you want me to collect xp from.`);
    };
    ...
    if (!avail.length){
      ...
      return message.channel.send(`${em.error} | **${message.member.displayName}**, The mentioned channels ${oldch} are not in the excempt list.`);
    };
