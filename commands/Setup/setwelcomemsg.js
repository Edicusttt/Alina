...
module.exports = {
  ...
  adminOnly: true,
  group: 'Setup',
  description: 'Set up the welcome message. Supports Embeds! [Learn how to set up your Welcome Message](https://discord.gg/uAVaeCP9VH)',
  ...
  run: (client, message, [stats = '', ...args]) => guilds.findById(message.guild.id, (err, doc) => {
    ...
    const source = 'https://discord.gg/uAVaeCP9VH';
    const variables = {
      '%TYPE': 'welcome',
      '%ACTION': 'joins',
    ...
    else if (stats === 'embed=set')
    {
      if (!args.length){
        ...
        return parser.saveDocument(doc, message, { type, source,
          ...
          subtitle: [
            status.success.map(x => `\\${em.success} ${x}`).join('\n'),
    ...
    else
    {
      ...
      else if (stats === 'test')
      {
        ...
        return message.channel.send(`${em.error} | **${message.author.tag}**, \`[${stats}]\` is not a valid option!`);
      };
    };
  })
};
