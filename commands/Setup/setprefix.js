...
module.exports = {
  ...
  run: (client, message, [prefix]) => guilds.findById(message.guild.id, (err, doc) => {
    ...
    if (!prefix){
      return message.channel.send(`${em.error} | **${message.author.tag}**, No new prefix detected! Please type the new prefix.`);
    } else if (prefix.length > 5){
      return message.channel.send(`${em.error} | **${message.author.tag}**, Invalid prefix. Prefixes cannot be longer than 5 characters!`);
    } else {
      ...
      .then(() => {
        ...
        return message.channel.send([
          `${em.success} | **${message.author.tag}**, Successfully`,
          [
