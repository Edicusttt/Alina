...
module.exports = {
  ...
  run: async (client, message, [id = '', ...reason]) => {
    ...
    if (!id.match(/\d{17,19}/)){
      return message.channel.send(`${em.error} | ${message.author}, Please provide the ID of the server you want me to leave from.`);
    };

    const guild = client.guilds.cache.get(id);

    if (!guild){
      return message.channel.send(`${em.error} | ${message.author}, guild **${id}** does not exist on your cache`)
    };
    ...
    .then(() => message.channel.send(`${em.success} | Sucessfully left the guild **${guild.name}**`))
