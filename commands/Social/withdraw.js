...
module.exports = {
  ...
  run: (client, message, [amount='']) => profile.findById(message.author.id, (err,doc) =>{
    ...
    if (err){
      ...
      return message.channel.send(`${em.error} | **${message.author.tag}**, You don't have a *bank* yet! To create one, type \`${client.prefix}bank\`.`);
    } else {
      ...
      if (!amount){
        return message.channel.send(`${em.error} | **${message.author.tag}**, [ **${amt}** ] is not a valid amount!.`);
      } else if (amount < 100){
        return message.channel.send(`${em.error} | **${message.author.tag}**, The amount to be withdrawn must be at least **100**.`);
      } else if (amount > doc.data.economy.bank){
        return message.channel.send([
          `${em.error} | **${message.author.tag}**, You don't have enough credits in your bank to proceed with this transaction.`,
        ...
        return message.channel.send(`${em.error} | **${message.author.tag}**, You can't withdraw this large sum of money (Overflow imminent)!`)
      };
      ...
      return doc.save()
      .then(() => message.channel.send(`${em.success} | **${message.author.tag}**, you successfully withdrawn **${text.commatize(amount * 0.95)}** credits from your bank! (+5% fee).`))
