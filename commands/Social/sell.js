...
module.exports = {
  ...
  run: (client, message, [id, amt] ) => profile.findById(message.author.id, async (err, doc) => {
    ...
    if (!itemcount || itemcount < amt){
      return message.channel.send(`${em.error} | **${message.author.tag}**, You do not have the necessary amount of **${item.name}** to sell.`);
    } else if (!item.price){
      return message.channel.send(`${em.error} | **${message.author.tag}**, Unable to sell ${item.name}.`);
    } else if (doc.data.economy.bank === null){
      return message.channel.send(`${em.error} | **${message.author.tag}**, You cannot sell items yet without a bank. Create one before selling items.`)
    } else {
      ...
      return doc.save()
      .then(() => message.channel.send(`${em.success} | **${message.author.tag}**, successfully sold **${amt}x ${item.name}** for **${text.commatize(total)}**`))
