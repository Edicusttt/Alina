...
module.exports = {
  ...
  run: (client, message, [id, amount]) => profile.findById(message.author.id, (err,doc) => {
    ...
    if (!item){
      ...
      return message.channel.send(`${em.error} | **${message.author.tag}**, item **${item.name}** is not deletable!`);
    };
    ...
    if (!amount){
       return message.channel.send(`${em.error} | **${message.author.tag}**, Invalid amount of item to be deleted!`);
    } else if (!itemcount || itemcount < amount){
      return message.channel.send(`${em.error} | **${message.author.tag}**, You do not have the necessary amount of **${item.name}** to delete.`);
    };
    ...
    .then(async () => {
      ...
      if (!proceed){
         return message.channel.send(`${em.error} | Cancelled operation!`);
      };

      return doc.save()
      .then(() => message.channel.send(`${em.success} | **${message.author.tag}**, Successfully deleted **${amount}x** of your **${item.name}!!**`))
