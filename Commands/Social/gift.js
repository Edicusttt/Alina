...
module.exports = {
  ...
  run: (client, message, [friend='', id, amount] ) => profile.findById(message.author.id, async (err, doc) => {
    ...
    if (err){
      ...
      return message.channel.send(`${em.error} | **${message.author.tag}**, you have no item in your inventory!`);
    };
    ...
    if (!friend){
      return message.channel.send(`${em.error} | **${message.author.tag}**, That friend could not be found!`);
    };
    ...
    if (!item){
      return message.channel.send(`${em.error} | **${message.author.tag}**, Could not find the item with id ${id}!`);
    } else if (!item.giftable){
      return message.channel.send(`${em.error} | **${message.author.tag}**, item **${item.name}** is not giftable!`);
    };
    ...
    return profile.findById(friend.id, (err, fr) => {
      ...
      if (err){
        ...
        return message.channel.send(`${em.error} | **${message.author.tag}**, **${friend.user.tag}** have no item in their inventory!`);
      };

      amount = Math.round(amount || 1);

      if (!amount){
        return message.channel.send(`${em.error} | **${message.author.tag}**, Invalid amount of item to be transferred!`)
      } else if (!itemcount || amount > itemcount){
        return message.channel.send(`${em.error} | **${message.author.tag}**, You don't have the necessary amount of **${item.name}** to complete this transaction!`);
      };
      ...
      .then(()=> message.channel.send(`${em.success} | **${message.member.displayName}**, successfully transferred **${amount}x ${item.name}** to **${friend.user.tag}**`))
      .catch(()=> message.channel.send(`${em.error} | [DATABASE_ERR]:\` The database responded with error: ${err.name}`))
    });
  })
};
