...
module.exports = {
  ...
  run: (client, message, [user='']) => profile.findById(message.author.id, async (err, tipper) => {
    ...
    if (err){
      return message.channel.send(`${em.error} | \`[DATABASE_ERR]:\` Unable to save the document to the database, please try again later!`);
    } else if (!tipper){
    ...
    if (tipper.data.tips.timestamp !== 0 && tipper.data.tips.timestamp - now > 0){
      return message.channel.send(`${em.error} | **${message.author.tag}**, you already used your tip. You can wait for ${moment.duration(tipper.data.tips.timestamp - now).format('H [hours,] m [minutes, and] s [seconds]')} to tip someone again.`);
    } else if (!user){
      return message.channel.send(`${em.success} | **${message.author.tag}**, you can now tip someone from this server!`);
    };
    ...
    if (!member){
      return message.channel.send(`${em.error} | **${message.author.tag}**, could not add tip to this user. Reason: User not found!`);
    } else if (member.id === message.author.id){
      return message.channel.send(`${em.error} | **${message.author.tag}**, you cannot give a tip to yourself!`);
    } else if (member.user.bot){
      return message.channel.send(`${em.error} | **${message.author.tag}**, you cannot tip a bot!`);
    };
    ...
    return profile.findById(member.id, async (err, doc) => {
      ...
      .then(() => message.channel.send([
        `${em.success} | **${message.author.tag}**, tipped **${amount}** to **${member.user.tag}**.`,
