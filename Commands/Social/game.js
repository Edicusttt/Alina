...
module.exports = {
  ...
  run: (client, message, [ title, ...args]) => profile.findById(message.author.id, (err, doc) => {
    ...
    if (err){
      ...
      return message.channel.send(`${em.error} | **${message.author.tag}**, register first before playing a game!`);
    } else if (!title){
      return message.channel.send(`${em.error} | **${message.author.tag}**, please provide the game you want to play.\n\n${categories}`);
    };

    const playGame = games.find(x => x[title])[title];

    if (!playGame){
      return message.channel.send(`${em.error} | **${message.author.tag}**, ${title} isn't a playable game. Please select from one below:\n\n${categories}`);
    };
    ...
    if (userprofile.date > now){
      return message.channel.send(`${em.error} | **${message.author.tag}**, please wait \`${momentduration}\` before playing **${title}** again.`);
    };
