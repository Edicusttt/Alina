  ...
  run: (client, message, [id, amt] ) => profile.findById(message.author.id, async (err, doc) => {
    ...
    if (!item){
      return message.channel.send([
        `${em.error} | **${message.author.tag}**, Could not find the item ${id ? `with id **${id}**` : `without id`}!`,
    ...
    if (!item.price && amt > 1){
      return message.channel.send(`${em.error} | **${message.author.tag}**, You may only have 1 free item at a time.`);
    } else if (amt > 1000){
      return message.channel.send(`${em.error} | **${message.author.tag}**, You cannot purchase more than **1,000** items at once.`);
    } else if (doc.data.economy.wallet < total){
      return message.channel.send([
        `${em.error} | **${message.author.tag}**, You do not have enough credits to proceed with this transaction!`,
      ...
      return doc.save()
      .then(() => message.channel.send(`${em.success} | **${message.author.tag}**, successfully purchased **${amt}x ${item.name}!**`))
