...
module.exports = {
  ...
  run: (client, message, [amount]) => profile.findById(message.author.id, (err, doc) => {
    ...
    if (err){
      ...
      if (!amount){
        return message.channel.send(`\\${em.error} | **${message.author.tag}**, [ **${amt || 0}** ] is not a valid amount!.`);
      } else if (amount < 100){
        return message.channel.send(`\\${em.error} | **${message.author.tag}**, The amount to be deposited must be at least **100**.`);
      } else if (amount * 1.05 > doc.data.economy.wallet){
        return message.channel.send([
          `\\${em.error} | **${message.author.tag}**, You don't have enough credits in your wallet to proceed with this transaction.`,
      ...
      return doc.save()
      .then(() => message.channel.send(`${em.success} | **${message.author.tag}**, you successfully deposited **${text.commatize(amount)}** credits to your bank! (+5% fee).`))
