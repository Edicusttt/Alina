...
module.exports = {
  ...
  run: async function (client, message, [member = '']){
    ...
    if (member.user.bot){
      ...
      return message.channel.send(`${em.error} | Bots cannot earn XP!`);
    };
    ...
    return profile.findOne({ _id: member.id }, async (err, doc) => {
      ...
      if (!doc || !doc.data.xp.some(x => x.id === message.guild.id)){
        return message.channel.send(`${em.error} | **${member.user.tag}** have not started earning XP in this server yet!`)
      };
      ...
      const color = doc.data.profile.color || 'rgb(153, 51, 255)'
      ...
      const def = await loadImage(doc.data.profile.background || 'https://i.imgur.com/35vshr1.jpeg');
