...
module.exports = {
  ...
  run: (client, message) => guilds.findById(message.guild.id, (err, doc) => {
    ...
    if (!channels.length){
      return message.channel.send(`${em.error} | **${message.member.displayName}**, Please mention the channel(s) you want me to not collect xp from.`);
    };
    ...
    if (!newch.length){
      ...
      return message.channel.send(`${em.error} | **${message.member.displayName}**, The mentioned channels ${oldch} are already in the excempt list.`);
    };
