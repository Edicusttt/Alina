...
module.exports = {
  ...
  run: async (client, message ) => {
    ...
    if (!continued){
      return message.channel.send(`${em.error} | **${message.author.tag}**, cancelled the xpreset command!`);
    };
    ...
    return profile.updateMany({'data.xp.id': message.guild.id }, {
      ...
      if (err){
        ...
        return message.channel.send(`${em.error} | **${message.author.tag}**, this server has no xp data to be cleared of!`);
      } else {
        return message.channel.send(`${em.success} | **${message.author.tag}**, this server's xp has been reset. (Cleared **${res.nModified}** xpdocs)`);
      };
    });
  }
};
