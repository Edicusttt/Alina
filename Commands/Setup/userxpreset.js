...
module.exports = {
  ...
  run: async (client, message ) => {
    ...
    if (!member){
      return message.channel.send(`${em.error} | **${message.author.tag}**, Couldn't find that member in this server!`);
    } else if (member.user.bot){
      return message.channel.send(`${em.error} | **${message.author.tag}**, A bot cannot earn experience points!`);
    };
    ...
    if (!continued){
      return message.channel.send(`${em.error} | **${message.author.tag}**, cancelled the userxpreset command!`);
    };
    ...
    return profile.findById(member.id, (err, doc) => {
      ...
      if (err){
        ...
        return message.channel.send(`${em.error}  **${message.author.tag}**, **${member.user.tag}** has not started earning xp!`);
      };
      ...
      return doc.save()
      .then(() => message.channel.send(`${em.success} | **${member.user.tag}**'s Experience Points has been sucessfully reset!`))
      .catch(() => message.channel.send(`${em.error} | **${member.user.tag}**'s Experience Points reset attempt failed!`))
    });
  }
};
