...
function error(message, options = {}){
  ...
  return message.channel.send(
    new MessageEmbed()
    .setColor('RED')
    .setFooter(`${options.type} | \©️${new Date().getFullYear()} Alina`)
    .setDescription([
      `${em.error}\u2000\u2000|\u2000\u2000${options.title}\n\n`,
      options.subtitle ? options.subtitle + '\n' : '',
      `[**Learn More**](${options.source}) on how to configure Alina's ${options.type} feature.`
    ].join(''))
  );
};
...
function saveDocument(document, message, options = {}){
  ...
  .then(() => message.channel.send(
    ...
    .setDescription([
      `\u2000\u2000|\u2000\u2000${options.title}\n\n`,
      options.subtitle ? options.subtitle + '\n' : '',
      `[**Learn More**](${options.source}) on how to configure Ailna's ${options.type} feature.`
    ].join(''))
  ));
};
